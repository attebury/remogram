...
## Two planning PR types

| Type | PR title prefix | `goal_branch.status` | Lifecycle changes |
|------|-----------------|----------------------|-------------------|
| **Draft packet** | `plan:draft` | stays **`draft`** | Create/refresh records only |
| **Approve packet** | `plan:approve` | **`draft` → `ready` → `approved`** via CLI | User must name exact `goal_branch_<id>` |

Draft PRs must not approve goals or promote requirements, tasks, verifications,
or decisions unless the user explicitly authorizes an **approve** PR.
...
`decision`, etc.) — never hand-edit `.tg` status lines.
...
- **Draft PR:** keep `goal_branch.status` **`draft`**; keep tasks unclaimed; keep plan steps pending.
- **Approve PR:** transitions only via `topogram sdlc transition`; PR title **`plan:approve …`**.
