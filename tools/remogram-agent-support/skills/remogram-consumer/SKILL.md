...
Fix `doctor` failures before trusting other packets. Doctor checks config schema, remote parsing, owner/repo match, trusted host binding, auth env **presence** (never values), provider capabilities, and **operator write overlay** bind when present.
...
Operators may grant writes without editing tracked `.remogram.json` using a bound local overlay (see `.remogram.operator.json.example` in the Remogram repo). Discovery order: **`--operator-config <path>`** → **`REMOGRAM_OPERATOR_CONFIG`** → **`$XDG_CONFIG_HOME/remogram/operator/<provider>-<owner>-<repo>.json`**. The overlay **`bind`** must match forge identity; mismatch → `config_invalid`. **`doctor --json`** adds **`operator_config`** and per-command **`source`** (`repo` | `operator` | `both` | `none`) under **`write_config`**.
...
`doctor --json` **`write_config`** warns when the provider supports writes your effective policy omits (repo and/or operator overlay).
