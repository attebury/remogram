...
3. Through **0.1.0-beta.3**, Remogram was read/plan only by default. Write commands are wired incrementally; consumers opt in via `.remogram.json` **`write_commands`**. **Agents must use CLI or MCP** for writes — direct `@remogram/provider-*` imports bypass the consumer gate (maintainer/tests only). **`cr open`** on **`gitea-api`** is the first write command; read scan limits from **`provider capabilities`** (`idempotency_scan`) before opening. Idempotency scans use ingest backoff; fail-closed `idempotency_scan_incomplete` includes `idempotency_scan` metadata — retry with **`cr inventory`** before manual open; concurrent opens retain a TOCTOU race. Merge execute remains out of scope.
