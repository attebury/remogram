...
export const ISSUE_INVENTORY_CURSOR_VERSION = 1;

/**
 * @param {{ sort: string, offset: number }} state
 * @returns {string}
 */
export function encodeIssueInventoryCursor(state) {
  ...
  const payload = JSON.stringify({ v: ISSUE_INVENTORY_CURSOR_VERSION, sort, offset });
 ...
 */
export function decodeIssueInventoryCursor(raw, opts = {}) {
  if (raw == null || raw === '') {
  ...
  if (decoded?.v !== ISSUE_INVENTORY_CURSOR_VERSION) {
