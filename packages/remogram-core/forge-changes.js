...
function baseIssueEventFields(issue) {
  return {
    issue_number: Math.floor(Number(issue.number)),
    title: sanitizeField(issue.title ?? '') || null,
    url: sanitizeUrl(issue.html_url ?? issue.url) || null,
  };
}
...
export function buildForgeChangesFromGiteaIssues(sinceIso, issuesArray) {
  const since = parseSinceObservedAt(sinceIso);
  const events = [];
  if (Array.isArray(issuesArray)) {
    for (const issue of issuesArray) {
      if (issue == null || issue.number == null) continue;
      const state = normalizePrState(issue.state);
      const base = baseIssueEventFields(issue);
      const createdAt = issue.created_at;
      const updatedAt = issue.updated_at;
      const closedAt = issue.closed_at;
      if (isAtOrAfter(createdAt, since)) {
        events.push({
          kind: FORGE_CHANGE_EVENT_KINDS.ISSUE_OPENED,
          ...base,
          state: 'open',
          opened_at: normalizeIsoTimestamp(createdAt),
        });
      ...
      const closedInWindow =
        isAtOrAfter(closedAt, since) ||
        (closedAt == null && state === 'closed' && isAtOrAfter(updatedAt, since));
      if (state === 'closed' && closedInWindow) {
        events.push({
          kind: FORGE_CHANGE_EVENT_KINDS.ISSUE_CLOSED,
          ...base,
          state: 'closed',
          closed_at: normalizeIsoTimestamp(closedAt ?? updatedAt),
        });
