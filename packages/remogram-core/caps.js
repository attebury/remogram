...
export function resolveWriteFieldMaxBytes(writeFieldPolicy = null) {
  if (writeFieldPolicy?.uncapped) return null;
  return writeFieldPolicy?.fieldMaxBytes ?? DEFAULT_FIELD_MAX_BYTES;
}

export function sanitizeWriteBodyWithMeta(value, writeFieldPolicy = null) {
  if (value == null) {
    return { value: null, truncated: false, original_bytes: 0, written_bytes: 0 };
  }
  const maxBytes = resolveWriteFieldMaxBytes(writeFieldPolicy);
  const cleaned = stripControlChars(value, { preserveNewlines: true }).trim();
  const redacted = redactSecretPatterns(cleaned);
  const original_bytes = Buffer.byteLength(redacted, 'utf8');
  const capped = capText(redacted, maxBytes);
  return {
    value: capped.text,
    truncated: capped.truncated,
    original_bytes,
    written_bytes: capped.bytes,
  };
}

export function sanitizeWriteTitleWithMeta(value, writeFieldPolicy = null) {
  if (value == null) {
    return { value: null, truncated: false, original_bytes: 0, written_bytes: 0 };
  }
  const maxBytes = resolveWriteFieldMaxBytes(writeFieldPolicy);
...
export function assertWriteFieldNotTruncated(meta, fieldName, writeFieldPolicy = null) {
  if (!meta?.truncated) return meta?.value ?? null;
  const capBytes = resolveWriteFieldMaxBytes(writeFieldPolicy);
  throw Object.assign(new Error(`${fieldName} exceeds active write field cap`), {
    forgeError: forgeError(
      ERROR_CODES.WRITE_FIELD_TRUNCATED,
      `${fieldName} exceeds active write field cap`,
      null,
      {
        field: fieldName,
...
export function sanitizeWriteBody(value, writeFieldPolicy = null) {
  return sanitizeWriteBodyWithMeta(value, writeFieldPolicy).value;
}
...
export function sanitizeWriteTitle(value, writeFieldPolicy = null) {
  return sanitizeWriteTitleWithMeta(value, writeFieldPolicy).value;
}
