...
export const FORGE_SOURCED_STRING_LEAVES = Object.freeze({
  ...
  issue_status: ['url', 'title', 'linked_change_request.url', 'linked_change_request.title'],
  issue_comments: ['comments[].id', 'comments[].author', 'comments[].body'],
  ...
  issue_inventory_slice: [
    'entries[].url',
    'entries[].title',
    'entries[].linked_change_request.url',
    'entries[].linked_change_request.title',
  ],
});
...
export const FACT_INVENTORY_BODY_SHAPES = Object.freeze({
  ...
  [FACT_INVENTORY_PACKET_TYPES.ISSUE_INVENTORY_SLICE]: {
    entries:
      'array<{ issue_number: number, url?: string, title?: string, state?: string, linked_change_request?: { pr_number: number, url?: string, state?: string, title?: string } }>',
    entry_count: 'number',
    truncated: 'boolean',
    list_truncated: 'boolean',
    has_more: 'boolean',
    ...
    slice_sort: 'string',
    entries_skipped:
      'array<{ issue_number: number, error_code: api_error | oversized_raw_output | ... }> optional',
    slice_ref: 'string optional',
  },
});
