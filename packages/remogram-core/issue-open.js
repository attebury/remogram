...
import { buildIssueViewBody } from './issue-view.js';
...
export function buildIssueOpenedBody(
  ...
  const body = reusedExisting
    ? buildIssueViewBody({ ...issue, title: resolvedTitle })
    : {
  ...
  if (reusedExisting) {
    ...
    body.created = true;
    body.body_bytes = Buffer.byteLength(String(issueBody ?? ''), 'utf8');
