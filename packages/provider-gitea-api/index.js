...
export async function repoStatus(ctx) {
  ...
  const remoteHeadRef = defaultBranch
    ? `${ctx.config.remote}/${defaultBranch}`
    : `${ctx.config.remote}/HEAD`;
  let configuredRemoteHead = null;
  try {
    configuredRemoteHead = gitRevParse(ctx.cwd, remoteHeadRef);
  } catch {}
  return {
    ...
    integration_ref_hints: {
      default_branch: defaultBranch,
      configured_remote_head: configuredRemoteHead,
    },
  };
}
...
export async function issueComments(ctx, { number }) {
  requireToken();
  if (number == null) {
    throw Object.assign(new Error('--number required'), {
      forgeError: forgeError(ERROR_CODES.INVALID_ARGS, 'Provide --number for issue comments'),
    });
  }
  const path = repoApiPath(ctx.config, 'issues', number, 'comments');
  const pageSep = path.includes('?') ? '&' : '?';
  let activeLimit = DEFAULT_CHECK_STATUS_PAGE_SIZE;
  const allComments = [];
  let listTruncated = false;
  ...
  for (let page = 1; page <= MAX_CHECK_PAGES; page += 1) {
    const { items, usedLimit } = await fetchPageWithIngestBackoff(
      async ({ page: pageNum, limit }) => {
        ...
        if (!Array.isArray(body)) {
          throw Object.assign(new Error('Provider returned non-array issue comments list'), {
            forgeError: forgeError(
              ERROR_CODES.UNPARSEABLE_PROVIDER_OUTPUT,
              'Provider returned non-array issue comments list',
            ),
          });
        }
        return body;
  ...
  const body = buildIssueCommentsFromGiteaComments(number, allComments);
  if (listTruncated) {
...
export async function forgeChanges(ctx, { since, include_issues = false } = {}) {
  requireToken();
  ...
  let anyListTruncated = listTruncated;
  let body = buildForgeChangesFromGiteaPulls(sinceIso, allPulls, {
    listTruncated: anyListTruncated,
  });
  if (include_issues) {
    const issuePath = `${repoApiPath(ctx.config, 'issues')}?state=all&sort=recentupdate`;
    let issueActiveLimit = GITEA_PAGE_SIZE;
    const allIssues = [];
    let issuesListTruncated = false;
    for (let page = 1; page <= MAX_CHECK_PAGES; page += 1) {
      const { items, usedLimit } = await fetchPageWithIngestBackoff(
        async ({ page: pageNum, limit }) => {
          const issueBody = await giteaFetch(
            ctx.config,
            ctx.parsed,
            `${issuePath}${pageSep}limit=${limit}&page=${pageNum}`,
          );
          if (!Array.isArray(issueBody)) {
            throw Object.assign(new Error('Provider returned non-array issue list'), {
              forgeError: forgeError(
                ERROR_CODES.UNPARSEABLE_PROVIDER_OUTPUT,
                'Provider returned non-array issue list',
              ),
            });
          }
          return issueBody;
      ...
      issueActiveLimit = usedLimit;
      allIssues.push(...items);
      if (items.length < usedLimit) break;
      if (page === MAX_CHECK_PAGES) {
        issuesListTruncated = true;
      }
    }
    anyListTruncated = anyListTruncated || issuesListTruncated;
    const issueEvents = buildForgeChangesFromGiteaIssues(sinceIso, allIssues);
    if (issueEvents.length > 0) {
      body = appendForgeChangeEvents(body, issueEvents, { listTruncated: anyListTruncated });
    }
  }
  ...
  if (checkEvents.length > 0) {
    body = appendForgeChangeEvents(body, checkEvents, { listTruncated: anyListTruncated });
  }
...
export async function getIssue(ctx, { number }) {
  if (number == null) {
    throw Object.assign(new Error('--number required'), {
      forgeError: forgeError(ERROR_CODES.INVALID_ARGS, 'Provide --number for issue lookup'),
    });
  }
  return giteaFetchIssueWriteResponse(ctx.config, ctx.parsed, repoApiPath(ctx.config, 'issues', number));
}

function linkedChangeRequestFromPull(pr) {
  const prNumber = Number(pr?.number);
  if (!Number.isInteger(prNumber) || prNumber <= 0) return null;
  const linked = { pr_number: prNumber };
  const url = sanitizeUrl(pr?.html_url ?? pr?.url);
  if (url) linked.url = url;
  const state = normalizeGiteaPrState(pr?.state);
  if (state) linked.state = state;
  const title = sanitizeField(pr?.title);
  if (title) linked.title = title;
  return linked;
}

function pullNumberFromIssue(issue) {
  ...
  const pullUrl = sanitizeUrl(issue?.pull_request?.url);
  if (!pullUrl) return null;
  const match = /\/pulls\/(\d+)$/.exec(pullUrl);
  if (!match) return null;
  const n = Number(match[1]);
  return Number.isInteger(n) && n > 0 ? n : null;
}

async function bestEffortLinkedChangeRequest(ctx, issue) {
  const pullNumber = pullNumberFromIssue(issue);
  if (!pullNumber) return null;
  try {
    const pull = await getPull(ctx, { number: pullNumber });
    return linkedChangeRequestFromPull(pull);
  } catch {
    return null;
  }
}
...
export async function crOpen(ctx, { head, base, title, body: prBody, idempotencyFingerprint = null }) {
  ...
  const titleMeta = sanitizeWriteTitleWithMeta(title, writeFieldPolicy);
  ...
  if (prBody != null && String(prBody).trim() !== '') {
    ...
    payload.body = assertWriteFieldNotTruncated(bodyMeta, 'body', writeFieldPolicy);
  }
...
export async function mergeExecute(ctx, { number, method = 'merge', expectedHeadSha, targetBranchRef = null }) {
  ...
  try {
    ...
    const isTimeoutOr5xx =
      (typeof status === 'number' && status >= 500)
      || String(message).toLowerCase().includes('timeout');
    if (isTimeoutOr5xx) {
      let targetBranchSha = null;
      let prState = null;
      let mergeCommitSha = null;
      let beforeBaseSha = null;
      try {
        const pr = await giteaFetch(
          ctx.config,
          ctx.parsed,
          repoApiPath(ctx.config, 'pulls', String(pullIndex)),
        );
        prState = pr?.state ?? pr?.merged ? 'closed' : null;
        mergeCommitSha = sanitizeField(pr?.merge_commit_sha ?? pr?.merged_commit_id ?? null);
        const targetRef = sanitizeField(targetBranchRef ?? pr?.base?.ref ?? null);
        beforeBaseSha = sanitizeField(pr?.base?.sha ?? null);
        if (targetRef) {
          targetBranchSha = await branchHeadSha(ctx, targetRef);
        }
      } catch {
        // bounded reconciliation only
      }
      const reconciliation = reconcileMergeEffectAfterError({
        expectedHeadSha: headCommitId,
        expectedBaseSha: beforeBaseSha,
        beforeBaseSha,
        targetBranchSha,
        prState,
        mergeCommitSha,
      });
      throw Object.assign(new Error(message || 'Merge endpoint failed'), {
        status,
        mergeIndeterminate: reconciliation,
        forgeError: forgeError(
          ERROR_CODES.MERGE_ENDPOINT_FAILED,
          sanitizeField(message) || 'Forge merge request failed',
          status,
        ),
      });
    }
  ...
  let commit_sha = sanitizeField(result?.sha ?? result?.merge_commit_sha ?? null);
  let base_sha = sanitizeField(result?.base_sha ?? null);
  let post_merge_readback = { status: 'proved' };

  const needsReadback = !commit_sha || !base_sha;
  if (needsReadback) {
    try {
      const pr = await giteaFetch(
        ctx.config,
      ...
      if (!commit_sha) {
        commit_sha = sanitizeField(pr?.merge_commit_sha ?? pr?.merged_commit_id ?? null);
      }
      const targetRef = sanitizeField(targetBranchRef ?? pr?.base?.ref ?? null);
      if (!base_sha && targetRef) {
        base_sha = await branchHeadSha(ctx, targetRef);
      }
      if (commit_sha && base_sha) {
        post_merge_readback = { status: 'proved' };
      } else if (base_sha) {
        post_merge_readback = { status: 'proved' };
      } else if (commit_sha) {
        post_merge_readback = {
          status: 'unavailable',
          reason: 'integration_base_unavailable',
        };
      } else {
        post_merge_readback = {
          status: 'unavailable',
          reason: 'post_merge_readback_incomplete',
        };
      }
    } catch (readbackErr) {
      post_merge_readback = {
        status: 'failed',
        reason: sanitizeField(readbackErr?.message) || 'post_merge_readback_failed',
      };
    }
  }
...
export async function listIssuesWithMeta(ctx, opts = {}) {
  requireToken();
  const sliceSort = normalizeCrInventorySort(opts.sort ?? DEFAULT_CR_INVENTORY_SLICE_SORT);
  const listLimit =
    opts.limit != null && Number.isInteger(Number(opts.limit)) && Number(opts.limit) > 0
  ...
  const retainMax =
    ...
    && Number(opts.retain_max) > 0
      ? Number(opts.retain_max)
      : null;
  const outputCap = listLimit ?? retainMax ?? null;
  let path = `${repoApiPath(ctx.config, 'issues')}?state=open`;
  path = appendSortQuery(path, giteaOpenPullSortQuery(sliceSort));
  const pageSep = path.includes('?') ? '&' : '?';
  let activeLimit = DEFAULT_OPEN_PULL_LIST_PAGE_SIZE;
  const all = [];
  let listTruncated = false;
  let entryCount = 0;
...
export async function listIssues(ctx, opts = {}) {
  const meta = await listIssuesWithMeta(ctx, opts);
  return meta.numbers;
}

export async function issueView(ctx, { number }) {
  requireToken();
  const issue = await getIssue(ctx, { number });
  const linkedChangeRequest = await bestEffortLinkedChangeRequest(ctx, issue);
  return buildIssueViewBody(issue, { linkedChangeRequest });
}

export async function issueInventorySlice(ctx, opts = {}) {
  return issueInventory(ctx, { listIssues, listIssuesWithMeta, issueView }, opts);
}
...
export async function prChecks(ctx, opts) {
  ...
  const { items: statusRecords, truncated: checks_truncated } = await (async () => {
    try {
      return await giteaFetchPaginated(
        ctx.config,
        ctx.parsed,
        repoApiPath(ctx.config, 'commits', sha, 'statuses'),
      );
    } catch (err) {
      const recovery = classifyCheckReadFailure(err, { prNumber: opts.number ?? null });
      if (recovery) {
        const base = err.forgeError
          ?? forgeError(ERROR_CODES.API_ERROR, err.message ?? 'Commit status read failed', err.status ?? null);
        throw Object.assign(new Error(base.message), {
          status: err.status ?? null,
          forgeError: withCheckReadRecovery(base, recovery),
        });
      }
      throw err;
    }
...
export const provider = {
  ...
  listIssues,
  listIssuesWithMeta,
  issueInventory: issueInventorySlice,
  prView,
  issueView,
  prChecks,
  mergePlan,
