import {
  ...
  buildReviewBundleBody,
  parseIssueBundleArgs,
  buildIssueBundleBody,
...
function invalidArgsError(message, context = {}) {
  return withInvalidArgsRemediation(
    forgeError(ERROR_CODES.INVALID_ARGS, message),
    context,
  );
}

export async function dispatchForgeCommand({ group, sub, flags, positional, ctx, provider }) {
  if (group === 'verify' && sub === 'bind') {
    return forgePacket(
      PACKET_TYPES.VERIFY_BIND,
      ctx,
      buildVerifyBindBody(parseVerifyBindArgs(flags)),
    );
  }
  if (group === 'review' && sub === 'bundle') {
    return forgePacket(
      PACKET_TYPES.REVIEW_BUNDLE,
      ctx,
      buildReviewBundleBody(parseReviewBundleArgs(flags)),
    );
  }
  if (group === 'issue' && sub === 'bundle') {
    return forgePacket(
      PACKET_TYPES.ISSUE_BUNDLE,
      ctx,
      buildIssueBundleBody(parseIssueBundleArgs(flags)),
    );
  }
  ...
  if (group === 'refs' && sub === 'compare') {
    if (!flags.base || !flags.head) {
      throw Object.assign(new Error('--base and --head required'), {
        forgeError: invalidArgsError('--base and --head required', { group, sub, flags }),
      });
    }
  ...
  if (group === 'issue' && sub === 'inventory') {
    if (typeof provider.issueInventory !== 'function') {
      throw Object.assign(new Error('issue inventory not implemented for provider'), {
        forgeError: forgeError(
          ERROR_CODES.PROVIDER_UNSUPPORTED,
          'issue inventory not implemented for provider',
        ),
      });
    }
    const inventoryBody = await provider.issueInventory(ctx, {
      slice_ref: flags.slice_ref,
      limit: parsePositiveInt(flags.limit, '--limit'),
      sort: flags.sort,
      cursor: flags.cursor,
    ...
    if (inventoryBody.list_truncated === true && !flags.cursor) {
      throw Object.assign(new Error('Open issue list incomplete'), {
        forgeError: forgeError(
          ERROR_CODES.INVENTORY_LIST_INCOMPLETE,
          'Open issue list could not be proved complete within pagination bounds',
          null,
          {
            inventory_list: {
              entry_count: inventoryBody.entry_count,
    ...
    return forgeFactInventoryPacket(
      FACT_INVENTORY_PACKET_TYPES.ISSUE_INVENTORY_SLICE,
      ctx,
      inventoryBody,
    );
  }
  if (group === 'cr' && sub === 'files') {
    ...
    if (number == null) {
      throw Object.assign(new Error('--number required'), {
        forgeError: invalidArgsError('--number required for cr files', { group, sub, flags }),
      });
    }
  ...
  if (group === 'cr' && sub === 'comments') {
    ...
    if (number == null) {
      throw Object.assign(new Error('--number required'), {
        forgeError: invalidArgsError('--number required for cr comments', { group, sub, flags }),
      });
    }
  ...
  if (group === 'issue' && sub === 'view') {
    ...
    if (number == null) {
      throw Object.assign(new Error('--number required'), {
        forgeError: invalidArgsError('--number required for issue view', { group, sub, flags }),
      });
    }
    if (typeof provider.issueView !== 'function') {
      throw Object.assign(new Error('issue view not implemented for provider'), {
        forgeError: forgeError(
          ERROR_CODES.PROVIDER_UNSUPPORTED,
          'issue view not implemented for provider',
        ),
      });
    }
    return forgePacket(PACKET_TYPES.ISSUE_STATUS, ctx, await provider.issueView(ctx, { number }));
  }
  if (group === 'issue' && sub === 'comments') {
    const number = parsePositiveInt(flags.number, '--number');
    if (number == null) {
      throw Object.assign(new Error('--number required'), {
        forgeError: invalidArgsError('--number required for issue comments', { group, sub, flags }),
      });
    }
    if (typeof provider.issueComments !== 'function') {
      throw Object.assign(new Error('issue comments not implemented for provider'), {
        forgeError: forgeError(
          ERROR_CODES.PROVIDER_UNSUPPORTED,
          'issue comments not implemented for provider',
        ),
      });
    }
    return forgePacket(
      PACKET_TYPES.ISSUE_COMMENTS,
      ctx,
      await provider.issueComments(ctx, { number }),
    );
  }
  if (group === 'forge' && sub === 'changes') {
    ...
    const body = await provider.forgeChanges(ctx, {
      since: sinceIso,
      include_issues: flags.include_issues === true,
    });
  ...
  if (group === 'cr' && sub === 'open') {
    ...
    parseCrOpenWriteArgs({ title: flags.title, body: flags.body }, writeFieldPolicy);
  ...
  if (group === 'issue' && sub === 'open') {
    ...
    if (!flags.title) {
      throw Object.assign(new Error('--title required'), {
        forgeError: invalidArgsError('--title required for issue open', { group, sub, flags }),
      });
    }
  ...
  if (group === 'status' && sub === 'set') {
    ...
    if (flags.number != null && (flags.sha == null || String(flags.sha).trim() === '')) {
      throw Object.assign(new Error('--sha required'), {
        forgeError: invalidArgsError('--sha required for status set', { group, sub, flags }),
      });
    }
  ...
  if (group === 'pr' && sub === 'view') {
    ...
    if (number == null) {
      throw Object.assign(new Error('--number required'), {
        forgeError: invalidArgsError('--number required for pr view', { group, sub, flags }),
      });
    }
  ...
  if (group === 'pr' && sub === 'checks') {
    ...
    if (number == null && !flags.ref) {
      throw Object.assign(new Error('--number or --ref required'), {
        forgeError: invalidArgsError('--number or --ref required for pr checks', {
          group,
          sub,
          flags,
        }),
  ...
  if (group === 'merge' && sub === 'plan') {
    ...
    if (number == null) {
      throw Object.assign(new Error('--number required'), {
        forgeError: invalidArgsError('--number required for merge plan', { group, sub, flags }),
      });
    }
  ...
  if (group === 'merge' && sub === 'execute') {
    ...
    if (number == null) {
      throw Object.assign(new Error('--number required'), {
        forgeError: invalidArgsError('--number required for merge execute', { group, sub, flags }),
      });
    }
    if (!flags.expected_base_sha || !flags.expected_head_sha) {
      throw Object.assign(new Error('expected SHAs required'), {
        forgeError: invalidArgsError(
    ...
    if (method !== 'merge') {
      throw Object.assign(new Error('Unsupported merge method'), {
        forgeError: invalidArgsError('Only --method merge is supported in v1', {
          group,
          sub,
          flags,
        }),
    ...
    try {
      ...
      if (err.mergeIndeterminate) {
        const reconciliation = {
          ...err.mergeIndeterminate,
          suggested_commands: err.mergeIndeterminate.suggested_commands.map((command) =>
            command.replace('<n>', String(number)),
          ),
        };
        ...
        return forgePacket(
          PACKET_TYPES.CR_MERGE_INDETERMINATE,
          ctx,
          buildCrMergeIndeterminateBody({
            prNumber: number,
            expected,
          ...
          }),
          {
            ...
            fields: {
              ...(fe.fields ?? {}),
              recovery: mergeEndpointRecoveryHints(number, expected),
            },
          },
        );
      }
      ...
      const withRecovery =
        fe.code === ERROR_CODES.MERGE_ENDPOINT_FAILED
          ? {
              ...fe,
              fields: {
                ...(fe.fields ?? {}),
  ...
  if (group === 'branch' && sub === 'protection') {
    ...
    if (!branchRef) {
      throw Object.assign(new Error('--branch-ref required'), {
        forgeError: invalidArgsError('--branch-ref required for branch protection', {
          group,
          sub,
          flags,
        }),
  ...
  if (group === 'contract' && (sub == null || sub === 'export')) {
    return forgePacket(
      PACKET_TYPES.COMMAND_CONTRACT_EXPORT,
      ctx,
      buildCommandContractBody(flags.command),
    );
  }

  throw Object.assign(new Error(`Unknown command: ${positional.join(' ')}`), {
    forgeError: invalidArgsError(
      'Unknown command. Try: provider capabilities, repo status, refs compare, refs inventory, cr inventory, issue inventory, cr files, cr comments, issue view, issue comments, cr open, issue open, issue bundle, status set, forge changes, verify bind, review bundle, pr view, pr checks, merge plan, merge execute, sync plan, whoami, branch protection, contract --command "<group subcommand>"',
      { group, sub, flags },
    ),
  });
}
