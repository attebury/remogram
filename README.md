...
`remogram provider capabilities --json` returns structured provider facts: which commands are implemented, per-command **`auth_class`**, auth env names, check sources, mergeability confidence, host binding, pagination, **`check_pagination`**, **`idempotency_scan`** (cr open scan limits and backoff), `forge_ingest_cap_bytes`, **`write_support`** (provider capability), and optional **`write_commands`**. Consumer **`.remogram.json`** must list **`write_commands`** to enable writes (fail closed otherwise).
