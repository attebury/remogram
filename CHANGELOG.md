...
- **Issue read commands:** `issue view`, `issue inventory`, `issue comments` with MCP tools ([#592](http://localhost:3000/attebury/remogram/pulls/592))
- **Issue open readback:** `issue_opened` packet with `body_bytes` and forge field honesty ([#592](http://localhost:3000/attebury/remogram/pulls/592))
- **Command contract export:** `contract export` / `COMMAND_REGISTRY` for CLI and MCP parity ([#592](http://localhost:3000/attebury/remogram/pulls/592))
- **Agent bundles:** `verify bind`, `review bundle`, `issue bundle` commands ([#592](http://localhost:3000/attebury/remogram/pulls/592), [#593](http://localhost:3000/attebury/remogram/issues/593))
- **Check read recovery:** classify Gitea status API timeouts/5xx and surface recovery hints ([#590](http://localhost:3000/attebury/remogram/issues/590))
- **Merge effect reconciler:** `cr_merge_indeterminate` packet when merge POST outcome is ambiguous ([#591](http://localhost:3000/attebury/remogram/issues/591))
- **`global-install-smoke.sh`:** pack-install smoke for core + providers + CLI ([#594](http://localhost:3000/attebury/remogram/issues/594))

### Changed

- **Invalid-args remediation:** structured hints on `INVALID_ARGS` for common CLI mistakes ([#592](http://localhost:3000/attebury/remogram/pulls/592))
- **Repo hints:** `integration_ref_hints` on `repo status` for dogfood alignment ([#592](http://localhost:3000/attebury/remogram/pulls/592))
- **Forge changes:** issue lifecycle events in `forge changes` delta ([#592](http://localhost:3000/attebury/remogram/pulls/592))
- **Merge recovery hints:** actionable merge-plan remediation when checks block ([#592](http://localhost:3000/attebury/remogram/pulls/592))

### Fixed

- **Contract registry:** register bundle commands in `COMMAND_REGISTRY` (25 commands in `contract export`) ([#593](http://localhost:3000/attebury/remogram/issues/593))
...
[0.1.0-beta.12]: https://github.com/attebury/remogram/releases/tag/v0.1.0-beta.12
