...
describe('remogram-mcp run-cli', () => {
  ...
  it('packetToMcpContent sanitizes ghs_ stderr on parse failure', () => {
    const token = 'ghs_abc1234567890';
    const out = packetToMcpContent('not json', `install error: ${token}`, 1, false);
    expect(JSON.stringify(JSON.parse(out.content[0].text))).not.toContain(token);
  });
