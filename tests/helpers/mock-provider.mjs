  ...
  return {
    ...
    providerCapabilities: async () => ({
      commands: [
        ...
        { name: 'issue_status', implemented: true, auth_class: 'token_required' },
        { name: 'issue_inventory', implemented: true, auth_class: 'token_required' },
        { name: 'issue_comments', implemented: true, auth_class: 'token_required' },
    ...
    repoStatus: async () => ({
      ...
      capabilities: ['repo_status', 'ref_compare', 'ref_inventory', 'cr_inventory', 'pr_status', 'pr_checks', 'merge_plan', 'sync_plan', 'whoami', 'branch_protection', 'cr_files', 'cr_comments', 'issue_status', 'issue_inventory', 'issue_comments', 'forge_changes', 'cr_open', 'status_set'],
      default_branch: 'main',
      integration_ref_hints: {
        default_branch: 'main',
        configured_remote_head: 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
      },
    }),
    ...
    issueInventory: async (_ctx, opts = {}) => ({
      entries: [
        {
          issue_number: 1,
      ...
      truncated: false,
      list_truncated: false,
      slice_sort: opts.sort ?? 'number_asc',
      has_more: false,
      complete: true,
      ...(opts.slice_ref ? { slice_ref: opts.slice_ref } : {}),
    }),
    issueComments: async (_ctx, { number }) => ({
      issue_number: number,
      comments: [
        {
          id: '101',
          author: 'issue-bot',
          body: 'Please include reproduction steps.',
        },
      ],
      comments_truncated: false,
      comment_count: 1,
