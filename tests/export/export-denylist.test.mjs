...
import { loadDogfoodSkillNames } from '../lib/dogfood-skills.mjs';
...
function applyExportDenylist(root) {
  ...
  for (const skill of loadDogfoodSkillNames(dogfoodList)) {
...
describe('export-public-main denylist', () => {
  ...
  it('P3 removes private paths from exported tree fixture including .cursor/skills', () => {
    ...
    mkdirSync(join(fixtureRoot, '.cursor/skills/remogram-dogfood'), { recursive: true });
    writeFileSync(join(fixtureRoot, 'topo/sdlc/secret.tg'), 'private\n');
    writeFileSync(join(fixtureRoot, 'README.md'), 'public\n');
    writeFileSync(
      join(fixtureRoot, '.cursor/skills/remogram-dogfood/SKILL.md'),
      'private skill\n'
    );
    ...
    expect(existsSync(join(fixtureRoot, '.cursor/skills/remogram-dogfood'))).toBe(false);
