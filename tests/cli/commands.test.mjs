...
describe('remogram cli commands', () => {
  ...
  it('verify bind returns verify_bind packet', async () => {
    const { cli } = env();
    const { logs } = await cli([
      'verify',
      'bind',
    ...
    const packet = JSON.parse(logs[0]);
    expectEnvelope(packet);
    expect(packet.type).toBe(PACKET_TYPES.VERIFY_BIND);
    expect(packet.bound).toBe(true);
  });

  it('review bundle returns review_bundle packet', async () => {
    const { cli } = env();
    const { logs } = await cli([
      'review',
      'bundle',
    ...
    const packet = JSON.parse(logs[0]);
    expectEnvelope(packet);
    expect(packet.type).toBe(PACKET_TYPES.REVIEW_BUNDLE);
    expect(packet.bundle_ready).toBe(true);
  });

  it('issue bundle returns issue_bundle packet', async () => {
    const { cli } = env();
    const { logs } = await cli([
      'issue',
      'bundle',
    ...
    const packet = JSON.parse(logs[0]);
    expectEnvelope(packet);
    expect(packet.type).toBe(PACKET_TYPES.ISSUE_BUNDLE);
    expect(packet.bundle_ready).toBe(true);
  });
  ...
  it('contract --command returns command contract export packet', async () => {
    const { cli } = env();
    const { logs } = await cli(['contract', '--command', 'issue inventory']);
    const packet = JSON.parse(logs[0]);
    expectEnvelope(packet);
    expect(packet.type).toBe(PACKET_TYPES.COMMAND_CONTRACT_EXPORT);
    expect(packet.command).toBe('issue inventory');
    expect(packet.found).toBe(true);
    expect(packet.contract.packet_type).toBe(PACKET_TYPES.ISSUE_INVENTORY_SLICE);
  });
