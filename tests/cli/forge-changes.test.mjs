...
describe('forge changes CLI', () => {
  ...
  it('passes --include-issues flag through to provider', async () => {
    const setup = setupTempForge({
      config: defaultTestConfig(),
      remoteUrl: 'https://localhost:3000/owner/repo.git',
    });
    cleanups.push(setup);
    let includeIssuesSeen = null;
    const provider = createMockProvider({
      forgeChanges: async (_ctx, { since, include_issues }) => {
        includeIssuesSeen = include_issues;
        return {
          since,
          since_kind: 'observed_at',
          events: [],
    ...
    const { logs } = await captureCliOutput(() =>
      runCli(
        ['forge', 'changes', '--since', '2024-06-01T12:00:00Z', '--include-issues', '--json'],
        {
          cwd: setup.dir,
          providers: { 'gitea-api': provider },
        },
    ...
    const packet = JSON.parse(logs[0]);
    expect(packet.ok).toBe(true);
    expect(includeIssuesSeen).toBe(true);
  });
});
