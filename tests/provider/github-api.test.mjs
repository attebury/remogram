...
import * as githubProvider from '@remogram/provider-github-api';
...
describe('provider-github-api fixtures', () => {
  beforeEach(() => {
    ...
    vi.spyOn(githubBranchProtection, 'resolveBranchProtection').mockResolvedValue({
      branch_ref: 'main',
  ...
  it('refsCompare resolves local refs and preserves shared packet body keys', async () => {
    ...
    expect(body.compare_base_ref).toBe('HEAD');
    expect(body.compare_head_ref).toBe('HEAD');
  ...
  it('prView maps mergeability via GraphQL and sanitizes fields', async () => {
    ...
    expect(body.forge_target_branch_ref).toBe('main');
    expect(body.forge_target_sha).toBe('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa');
    expect(body.forge_source_sha).toBe('bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb');
  ...
  it('prChecks supports a local git ref without fetching the pull request', async () => {
    ...
    expect(body.forge_source_sha).toMatch(/^[0-9a-f]{40}$/);
    expect(body.check_conclusion).toBe('success');
    expect(global.fetch).toHaveBeenCalledTimes(2);
  });
  ...
  it('listOpenPullsWithMeta preserves search total_count on number_asc fallback', async () => {
    ...
    global.fetch
      ...
      .mockResolvedValueOnce(jsonResponse(allMrs.slice(3)));
