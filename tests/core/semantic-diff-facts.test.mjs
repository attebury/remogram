...
describe('semantic diff fact inventory contracts', () => {
  ...
  it('registers ref_inventory, cr_inventory_slice, and issue_inventory_slice packet types', () => {
  ...
  it('v1 read/plan commands include fact inventory CLI surfaces', () => {
    expect(V1_READ_PLAN_COMMANDS).toEqual(
      expect.arrayContaining(['refs inventory', 'cr inventory', 'issue inventory', 'issue view', 'issue comments']),
    );
  });
  ...
  it('observer fact inventory registry aligns with v1 command map', () => {
    ...
    expect(observerTypes).toContain(FACT_INVENTORY_PACKET_TYPES.ISSUE_INVENTORY_SLICE);
    expect(allObserverEligibleCommands().map((entry) => entry.command)).toEqual(
      expect.arrayContaining(['refs inventory', 'cr inventory', 'issue inventory']),
    );
  });
});
