...
describe('write config gate', () => {
  ...
  it('assertWriteCommandConfigured throws write_not_configured', () => {
    ...
    expect(writeNotConfiguredMessage('cr_open')).toMatch(/operator config/i);
  });

  it('assertWriteCommandConfigured accepts effective write policy overlay', () => {
    const policy = {
      repoWriteCommands: [],
    ...
    expect(() => assertWriteCommandConfigured(policy, 'merge')).not.toThrow();
  });
  ...
  it('parseConfigFile accepts write_commands cr_open and status_set', () => {
    const config = parseConfigFile(
      JSON.stringify({
        ...
        owner: 'o',
        repo: 'r',
        write_commands: ['cr_open', 'status_set', 'issue_open'],
      }),
    );
    expect(config.write_commands).toEqual(['cr_open', 'status_set', 'issue_open']);
  });

  it('parseConfigFile accepts write_commands merge', () => {
    const config = parseConfigFile(
      JSON.stringify({
        version: '1',
        provider: 'gitea-api',
    ...
    expect(config.write_commands).toEqual(['merge']);
  });
