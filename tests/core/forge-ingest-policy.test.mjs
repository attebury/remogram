...
describe('forge ingest cap policy', () => {
  ...
  it('honors REMOGRAM_FORGE_INGEST_MAX_BYTES when valid', () => {
    ...
    expect(forgeIngestCapabilityFacts()).toEqual({
      forge_ingest_cap_bytes: 4096,
      forge_ingest_env_override: true,
    });
  });
  ...
  it('clamps env override above MAX_FORGE_INGEST_ENV_BYTES', () => {
    process.env[FORGE_INGEST_MAX_BYTES_ENV] = '999999';
    expect(getEffectiveIngestMaxBytes()).toEqual({
      bytes: 65536,
      envOverride: true,
      clamped: true,
    ...
    expect(forgeIngestCapabilityFacts()).toEqual({
      forge_ingest_cap_bytes: 65536,
      forge_ingest_env_override: true,
      forge_ingest_cap_clamped: true,
    });
  });
