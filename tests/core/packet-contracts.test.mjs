...
describe('packet contracts', () => {
  ...
  it('issue_status', () => {
    const p = forgePacket(PACKET_TYPES.ISSUE_STATUS, ctx, {
      issue_number: 12,
      url: 'http://localhost:3000/o/r/issues/12',
      state: 'open',
      title: 'Bug report',
      linked_change_request: { pr_number: 77, url: 'http://localhost:3000/o/r/pulls/77' },
    });
    expect(p.type).toBe(PACKET_TYPES.ISSUE_STATUS);
    expect(bodyKeys(p)).toEqual(['issue_number', 'linked_change_request', 'state', 'title', 'url']);
  });

  it('issue_comments', () => {
    const p = forgePacket(PACKET_TYPES.ISSUE_COMMENTS, ctx, {
      issue_number: 1,
      comments: [{ id: '1', author: 'a', body: 'x' }],
      comments_truncated: false,
      comment_count: 1,
    });
    expect(p.type).toBe(PACKET_TYPES.ISSUE_COMMENTS);
    expect(bodyKeys(p)).toEqual(['comment_count', 'comments', 'comments_truncated', 'issue_number']);
  });

  it('issue_inventory_slice', () => {
    const p = forgePacket(PACKET_TYPES.ISSUE_INVENTORY_SLICE, ctx, {
      entries: [{ issue_number: 1, state: 'open' }],
      entry_count: 1,
      entry_count_observed: 1,
      truncated: false,
      list_truncated: false,
      slice_sort: 'number_asc',
      has_more: false,
      complete: true,
    });
    expect(p.type).toBe(PACKET_TYPES.ISSUE_INVENTORY_SLICE);
    expect(bodyKeys(p)).toEqual([
      'complete',
      'entries',
      'entry_count',
      'entry_count_observed',
      'has_more',
      'list_truncated',
      'slice_sort',
      'truncated',
  ...
  it('command_contract_export', () => {
    const p = forgePacket(PACKET_TYPES.COMMAND_CONTRACT_EXPORT, ctx, {
      command: 'issue view',
      found: true,
      contract: {
        command: 'issue view',
        packet_type: 'issue_status',
        read_only: true,
      },
    });
    expect(p.type).toBe(PACKET_TYPES.COMMAND_CONTRACT_EXPORT);
    expect(bodyKeys(p)).toEqual(['command', 'contract', 'found']);
  });

  it('verify_bind', () => {
    const p = forgePacket(PACKET_TYPES.VERIFY_BIND, ctx, {
      target_sha: HEAD,
      verifier: 'verify-lane',
      bound: true,
    });
    expect(p.type).toBe('verify_bind');
    expect(bodyKeys(p)).toEqual(['bound', 'target_sha', 'verifier']);
  });

  it('review_bundle', () => {
    const p = forgePacket(PACKET_TYPES.REVIEW_BUNDLE, ctx, {
      pr_number: 7,
      reviewed_head_sha: HEAD,
      decision: 'approved',
      bundle_ready: true,
    });
    expect(p.type).toBe('review_bundle');
    expect(bodyKeys(p)).toEqual(['bundle_ready', 'decision', 'pr_number', 'reviewed_head_sha']);
  });

  it('issue_bundle', () => {
    const p = forgePacket(PACKET_TYPES.ISSUE_BUNDLE, ctx, {
      issue_number: 514,
      state: 'closed',
      linked_pr: 7,
      bundle_ready: true,
    });
    expect(p.type).toBe('issue_bundle');
    expect(bodyKeys(p)).toEqual(['bundle_ready', 'issue_number', 'linked_pr', 'state']);
  });
