...
describe('observer fact inventory contract', () => {
  ...
  it('includes semantic-diff fact inventory commands as read-only extensions', () => {
    ...
    expect(extended.some((entry) => entry.command === 'issue inventory')).toBe(true);
    expect(extended.some((entry) => entry.command === 'issue view')).toBe(true);
    expect(extended.some((entry) => entry.command === 'issue comments')).toBe(true);
  ...
  it('covers all v1 read/plan commands in observer eligibility map', () => {
    ...
    expect(commands).toContain('issue inventory');
