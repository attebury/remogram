...
describe('merge execute helpers', () => {
  ...
  it('buildMergeExecuteAfterFacts does not reuse pre-merge base when readback missing (#587)', () => {
    const view = {
      forge_target_sha: BASE,
      forge_source_sha: HEAD,
      mergeability: 'clean',
    };
    const after = buildMergeExecuteAfterFacts(view, {
      base_sha: null,
      post_merge_readback: { status: 'failed', reason: 'post_merge_readback_failed' },
    });
    expect(after.base_sha).toBeNull();
    expect(after.post_merge_readback.status).toBe('failed');
  });

  it('buildMergeExecuteAfterFacts uses provider base_sha when readback proved (#587)', () => {
    const POST_MERGE_BASE = 'cccccccccccccccccccccccccccccccccccccccc';
    const view = {
      forge_target_sha: BASE,
      forge_source_sha: HEAD,
      mergeability: 'clean',
    ...
    const after = buildMergeExecuteAfterFacts(view, {
      base_sha: POST_MERGE_BASE,
      post_merge_readback: { status: 'proved' },
    });
    expect(after.base_sha).toBe(POST_MERGE_BASE);
  });

  it('buildMergeExecuteMergeFacts carries post_merge_readback metadata', () => {
    const merge = buildMergeExecuteMergeFacts('merge', {
      commit_sha: HEAD,
      provider_status: 200,
      post_merge_readback: { status: 'proved' },
    });
    expect(merge.post_merge_readback).toEqual({ status: 'proved' });
  });
