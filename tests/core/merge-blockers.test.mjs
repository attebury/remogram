...
describe('mergeBlockersFromFacts', () => {
  ...
  it('blocks unnormalizable changed paths as unavailable not path_scope_violation', () => {
    expect(
      mergeBlockersFromFacts(openClean, checksSuccess, {
        allowed_paths: ['packages/**'],
    ...
    expect(
      mergeBlockersFromFacts(openClean, checksSuccess, {
        allowed_paths: ['packages/**'],
        changed_paths: ['topo/../packages/x.js'],
      }),
    ).toEqual(['changed_paths_unavailable']);
  });

  it('vacuously passes empty changed_paths when allowlist is configured', () => {
    expect(
      mergeBlockersFromFacts(openClean, checksSuccess, {
        allowed_paths: ['packages/**'],
        changed_paths: [],
  ...
  it('honors merge policy for missing and pending conclusions', () => {
    const policyBoth = { allow_missing_checks: true, allow_pending_checks: true };
    expect(mergeBlockersFromFacts(openClean, { check_conclusion: 'missing' }, {}, policyBoth)).toEqual(
      [],
    );
    expect(mergeBlockersFromFacts(openClean, { check_conclusion: 'pending' }, {}, policyBoth)).toEqual(
      [],
    );
    expect(
      mergeBlockersFromFacts(openClean, { check_conclusion: 'failure' }, {}, policyBoth),
    ).toEqual(['checks_failed']);
    expect(
      mergeBlockersFromFacts(
        openClean,
        { check_conclusion: 'success', checks_truncated: true },
        {},
        policyBoth,
      ),
    ).toEqual(['checks_incomplete']);
