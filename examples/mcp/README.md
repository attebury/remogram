...
`doctor`, `provider_capabilities`, `repo_status`, `ref_compare`, `ref_inventory`, `cr_inventory`, `whoami`, `branch_protection`, `cr_files`, `cr_comments`, `forge_changes`, `cr_open`, `status_set`, `pr_status`, `pr_checks`, `merge_plan`, `sync_plan` — same JSON as `remogram … --json`.
