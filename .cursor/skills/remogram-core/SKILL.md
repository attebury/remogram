...
**v1 scope:** Through **0.1.0-beta.4**, read/plan only by default. **`write_commands`** lists consumer write ids — CLI/MCP only. Provider write entrypoints also call **`assertWriteCommandConfigured`** (defense in depth); direct `@remogram/provider-*` imports remain maintainer/tests only. Not listed → `write_not_configured`; consumers use forge/CI outside Remogram for those writes. Read **`idempotency_scan`** from **`remogram provider capabilities --json`** before **`cr open`** or **`status set`**. Idempotency scan fails closed with `idempotency_scan_incomplete`; for CR open retry with **`cr inventory`**. **`REMOGRAM_FORGE_INGEST_MAX_BYTES`** env override is capped at 65536; `doctor` warns when the agent-safe default is weakened; capabilities may include **`forge_ingest_env_override`** and **`forge_ingest_cap_clamped`** when the env is set. Merge execute remains out of scope.
